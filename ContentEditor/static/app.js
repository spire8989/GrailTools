...
function synthAudioOptions(kind, current, includeInherit = false, inheritLabel = "Inherit travel music") {
  ...
  const firstOption = includeInherit
    ? `<option value="__inherit__"${current === undefined ? " selected" : ""}>${escapeHtml(inheritLabel)}</option><option value=""${current === null ? " selected" : ""}>None</option>`
...
function renderSynthAudioSelect(label, current, kind, attributes, includeInherit = false, inheritLabel = "Inherit travel music") {
  return `<label class="asset-selector wide"><span>${escapeHtml(label)}</span><span class="asset-selector-controls"><select ${attributes}>${synthAudioOptions(kind, current, includeInherit, inheritLabel)}</select></span><span class="hint">Synthesized ${kind === "music" ? "music track" : "SFX"}; no uploaded file is required.</span></label>`;
}

function renderSynthAudioSelector(label, field, current, kind, includeInherit = false, inheritLabel = "Inherit travel music") {
  ...
  return renderSynthAudioSelect(label, selectedValue, kind, `data-field="${escapeHtml(field)}"`, includeInherit, inheritLabel);
}
...
function syncCharacterVisualPreviews() {
  ...
  document.querySelectorAll("[data-character-preview]").forEach((root) => {
    ...
    const previewSlot = root.dataset.previewSlot;
    const visual = ["idle", "walk", "attack"].includes(previewSlot)
      ? state.draft.visuals?.[previewSlot] || {}
      : state.draft.visuals?.animations?.[previewSlot] || {};
...
function renderEnemySpecialAnimations(definition, context) {
  const animations = definition?.visuals?.animations || {};
  const rows = Object.entries(animations).map(([animationId, visual]) => {
    const entry = visual && typeof visual === "object" ? visual : {};
    const impactFrame = `<label>Impact frame<input type="number" min="0" step="1" data-special-animation-field="impactFrame" data-special-animation-id="${escapeHtml(animationId)}" value="${escapeHtml(entry.impactFrame ?? "")}" placeholder="default"><span class="hint">default: about 60% of frames</span></label>`;
    const path = `visuals.animations.${animationId}`;
    return `<div class="section-card"><h4>${escapeHtml(animationId)}</h4>${renderCharacterVisualPreview(animationId, `Special: ${animationId}`, entry, { characterId: definition?.id })}${renderAssetSelector("Sprite sheet", `${path}.assetId`, entry.assetId, "image", "combat", `${context} ${animationId}`, "sprite_sheet")}<div class="form-grid four"><label>Animation ID<input data-special-animation-id="${escapeHtml(animationId)}" value="${escapeHtml(animationId)}" pattern="[a-z][a-z0-9_]*"><span class="hint">lowercase letters, numbers, and underscores</span></label><label>Frames<input type="number" min="1" step="1" data-special-animation-field="frameCount" data-special-animation-id="${escapeHtml(animationId)}" value="${escapeHtml(entry.frameCount ?? "")}" placeholder="optional"></label><label>Columns<input type="number" min="1" step="1" data-special-animation-field="columns" data-special-animation-id="${escapeHtml(animationId)}" value="${escapeHtml(entry.columns ?? "")}" placeholder="optional"></label><label>FPS<input type="number" min="0" step="any" data-special-animation-field="fps" data-special-animation-id="${escapeHtml(animationId)}" value="${escapeHtml(entry.fps ?? "")}" placeholder="default"></label><label>Scale<input type="number" min="0.25" max="3" step="0.05" data-special-animation-field="scale" data-special-animation-id="${escapeHtml(animationId)}" value="${escapeHtml(entry.scale ?? "")}" placeholder="1"></label>${impactFrame}<label>Offset X<input type="number" step="1" data-special-animation-field="offsetX" data-special-animation-id="${escapeHtml(animationId)}" value="${escapeHtml(entry.offsetX ?? "")}" placeholder="0"><span class="hint">normalized px; + right</span></label><label>Offset Y<input type="number" step="1" data-special-animation-field="offsetY" data-special-animation-id="${escapeHtml(animationId)}" value="${escapeHtml(entry.offsetY ?? "")}" placeholder="0"><span class="hint">normalized px; + down</span></label></div><div class="button-row"><button type="button" class="small-button danger-outline" data-action="remove-enemy-special-animation" data-enemy-special-animation-id="${escapeHtml(animationId)}">Remove</button></div></div>`;
  }).join("");
  return `<section class="section character-visuals enemy-special-animations"><div class="section-heading"><div><h3>Special Animations</h3><p>Optional named enemy sprite-sheet animations. Enemy actions reference these IDs and fall back to the default Attack animation when unavailable.</p></div><button type="button" class="small-button" data-action="add-enemy-special-animation">Add animation</button></div>${rows || `<p class="hint">No special animations. Enemy actions will use Attack.</p>`}</section>`;
}
...
function renderGlobalSettings() {
  ...
  const musicDucking = audio.musicDucking || {};
...
function renderMinigameHotspotCard(hotspot, index) {
  return `<details class="minigame-hotspot-card" data-minigame-hotspot-card="${index}" open><summary><strong>${escapeHtml(hotspot.name || hotspot.id || `Hotspot ${index + 1}`)}</strong><span class="panel-count">Priority ${escapeHtml(hotspot.priority ?? 0)}</span></summary><div class="form-grid three">
    ...
    ${minigameNumberInput("Hook window (ms)", "hookWindowMs", hotspot.hookWindowMs, index, "1", "800")}
  ...
  </div><div class="button-row"><button type="button" class="small-button" data-action="duplicate-minigame-hotspot" data-minigame-hotspot-index="${index}">Duplicate hotspot</button><button type="button" class="small-button danger-outline" data-action="remove-minigame-hotspot" data-minigame-hotspot-index="${index}">Remove hotspot</button></div></details>`;
}
...
function renderMinigame() {
  ...
  return `<div class="editor-title"><div><h2>${escapeHtml(minigame.name || minigame.id || "New minigame")}</h2><p>${escapeHtml(minigame.id || "Unsaved ID")} · Fishing</p></div><span class="schema-badge">Minigame schema</span></div>
    ...
    <section class="section"><div class="section-heading"><div><h3>Tutorial text</h3><p>The tutorial is authored with the minigame and shown by the game runtime when the session begins.</p></div></div><div class="form-grid"><label class="wide"><input type="checkbox" data-minigame-tutorial-field="enabled" ${tutorial.enabled === true ? "checked" : ""}> Show tutorial intro when this minigame starts</label><label>Tutorial title<input data-minigame-tutorial-field="title" value="${escapeHtml(tutorial.title || "")}"></label><label class="wide">Tutorial instructions<textarea data-minigame-tutorial-field="text">${escapeHtml(tutorial.text || "")}</textarea></label><label class="wide">Completion text<textarea data-minigame-tutorial-field="completionText">${escapeHtml(tutorial.completionText || "")}</textarea></label></div></section>
...
function defaultEntry(category) {
  if (category === "playerCharacter") return { id: "arthur", name: "Arthur", portraitAssetId: null, combatVisualAssetId: null, provisionCapacity: 20, materialBagCapacity: 15, provisionConsumptionMultiplier: 1, combat: { maxHp: 45, speed: 10 } };
  ...
  if (category === "minigames") return {
    ...
    defaultWater: {
      biteChance: 0.55, biteDelayMin: 0.8, biteDelayMax: 1.6, hookWindowMs: 1350,
...
function commitEnemySpecialAnimationId(input) {
  const oldId = input.dataset.specialAnimationId;
  const newId = input.value.trim();
  const animations = state.draft?.visuals?.animations || {};
  if (!newId || newId === oldId || !/^[a-z][a-z0-9_]*$/.test(newId)
    || Object.prototype.hasOwnProperty.call(animations, newId)) {
    input.value = oldId;
    return;
  }
  const nextAnimations = {};
  Object.entries(animations).forEach(([id, visual]) => {
...
function handleInput(input) {
  ...
  if (input.dataset.specialAnimationField) {
    const animation = state.draft.visuals?.animations?.[input.dataset.specialAnimationId];
    if (!animation) return;
    const value = parseInputValue(input, input.dataset.specialAnimationField);
    if (value === undefined || value === "") delete animation[input.dataset.specialAnimationField];
    else animation[input.dataset.specialAnimationField] = value;
    markDirty();
    return;
  }
...
function handleAction(button) {
  ...
  if (action === "select-minigame-hotspot") {
    ...
    state.draft.hotspots.push({
      ...
      hookWindowMs: state.draft.defaultWater?.hookWindowMs ?? 1350,
    ...
    markDirty();
    render();
  } else if (action === "duplicate-minigame-hotspot") {
    const index = Number(button.dataset.minigameHotspotIndex);
    const hotspot = state.draft?.hotspots?.[index];
    if (!hotspot) return;
    const hotspots = state.draft.hotspots;
    const usedIds = Object.fromEntries(hotspots.map((entry) => [entry.id, true]));
    const copy = clone(hotspot);
    copy.id = uniqueId(`${hotspot.id || "hotspot"}_copy`, usedIds);
    const originalName = String(hotspot.name || "Water Hotspot").trim() || "Water Hotspot";
    const existingNames = new Set(hotspots.map((entry) => String(entry.name || "").trim()));
    const copiedName = `${originalName} Copy`;
    let nextName = copiedName;
    let suffix = 2;
    while (existingNames.has(nextName)) nextName = `${copiedName} ${suffix++}`;
    copy.name = nextName;
    copy.x = clampTownLayoutValue(Number(hotspot.x) > 0.96 ? Number(hotspot.x) - 0.04 : Number(hotspot.x) + 0.04, hotspot.x);
    copy.y = clampTownLayoutValue(Number(hotspot.y) > 0.96 ? Number(hotspot.y) - 0.04 : Number(hotspot.y) + 0.04, hotspot.y);
    hotspots.splice(index + 1, 0, copy);
    state.minigameHotspotSelectedId = copy.id;
    markDirty();
    render();
    ...
    markDirty();
    render();
  } else if (action === "add-enemy-special-animation") {
    state.draft.visuals ||= {};
    state.draft.visuals.animations ||= {};
    const animationId = uniqueId("new_animation", state.draft.visuals.animations);
    state.draft.visuals.animations[animationId] = {};
    markDirty();
    render();
...
document.addEventListener("change", (event) => {
  ...
  else if (event.target.matches("[data-special-animation-id]")) commitEnemySpecialAnimationId(event.target);
