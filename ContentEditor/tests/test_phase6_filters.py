...
class Phase6FilterBrowserTests(unittest.TestCase):
    ...
    def test_town_layout_editor_renders_and_updates_normalized_hotspot(self) -> None:
        ...
        self.assertEqual(self.browser.evaluate("document.querySelector('[data-town-layout-style]').value"), "plaque")
        self.browser.evaluate("(() => { const select=document.querySelector('[data-town-layout-style]'); select.value='seal'; select.dispatchEvent(new Event('change',{bubbles:true})); })()")
        self.assertTrue(self.browser.evaluate("[...document.querySelectorAll('[data-town-layout-marker]')].every(marker=>marker.classList.contains('town-hotspot-style-seal'))"))
        self.assertFalse(self.browser.evaluate("state.dirty"))
